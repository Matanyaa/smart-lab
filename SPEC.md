...
- 2026-09-17 — Added a 60s `setInterval` poll to the update-check (on top of load/visibilitychange/pageshow) after testing showed a tab left open in the foreground the whole time never re-checks otherwise — same limitation exists in `calorie-tracker`/`split-builder` today, just not one the user had hit there. Worth carrying this addition back to those apps too if it matters there. Bumped to `0.1.0-t04`.
