...
import { newTerminalNode, newContainerNode } from './workflow.js?v=1.0.0';
...
// parameters}` in `actionDefs` -- `parentId` is now **hierarchy-
...
export async function loadCatalog() {
  const snap = await getDocs(collection(db, 'actionDefs'));
  actionDefs = [];
  ...
  addBtn.addEventListener('click', async () => {
    ...
    try {
      const parameters = kindSelect.value === 'terminal'
        ...
        : [];
      await addDoc(collection(db, 'actionDefs'), {
...
async function deleteActionDefCascade(id) {
  ...
  await deleteDoc(doc(db, 'actionDefs', id));
}
